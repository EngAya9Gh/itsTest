                                ...
                                <table class="table table-hover js-basic-example dataTable table-custom mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td><img src="{{asset('assets/images/fatura/'.$service->image)}}" data-toggle="tooltip" data-placement="top" title="Team Lead" alt="Avatar" class="width35 rounded"></td>
                                      @if(auth()->user()->role==1)
                                         ...
                                         @endif 
                                            ...
                                            <td class="project-actions">
                                                ...
                                                <a href="/fatura/{{$service->id}}/section " class="btn btn-sm btn-outline-primary"><i class="icon-eye"></i></a>
                                               @if(auth()->user()->role==1)
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body"> 
                <form method="Post" action="{{ route('fatura-section.store') }}" enctype="multipart/form-data">
               ...
               <div class="input-group mb-3">
                        <div class="input-group-prepend">
                            <span class="input-group-text">نسبة الربح من السعر الاساسي(%)
</span>
                        </div>
                        <input type="text" class="form-control" required value="10"  name="increase_percentage" aria-label="increase_percentage" aria-describedby="basic-addon2">
                    </div>
...
<div class="modal fade" id="deleteModal{{$service->id}}" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body"> 
              <form action="{{ route('fatura-section.destroy', $service->id) }}" method="POST">
               @csrf
               @method('DELETE')
               <input type="hidden" name="_token" value="{{ csrf_token() }}" />
               <div class="modal-footer">
...
<div class="modal fade" id="editModal{{$service->id}}" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body"> 
                <form method="POST" action="{{ route('fatura-section.update',  $service->id) }}" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    {{ method_field('PATCH') }}
          ...
          <div class="input-group mb-3">
                        ...
                        <input type="text" class="form-control" required value="{{$service->increase_percentage}}"  name="increase_percentage" aria-label="increase_percentage" aria-describedby="basic-addon2">
                    </div>
...
<div class="modal fade" id="enableModal{{$service->id}}" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body"> 
              <form action="/fatura-section/{{$service->id}}/status" method="POST">
               @csrf
               <input type="hidden" name="_token" value="{{ csrf_token() }}" />
               <div class="modal-footer">
                   <button type="submit" class="btn btn-primary">نعم</button>
