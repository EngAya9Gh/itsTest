...
<td class="project-title">
    ...
    <td class="project-actions">
     ...
     <a href="javascript:void(0);" data-toggle="modal" data-target="#viewModal{{$serviceOrder->id}}"class="btn btn-sm btn-outline-primary"><i class="icon-eye"></i></a>
      ...
      @if($serviceOrder->status==1 &&   $serviceOrder->service_category_name!='IMEI' &&  $serviceOrder->service_category_name!='SERVER'  )
...
<!--------------Reject -------------->
@foreach ($serviceOrders as $key => $serviceOrder)
 @if(   $serviceOrder->service_category_name!='IMEI' &&  $serviceOrder->service_category_name!='SERVER'  )
...
<script>
// عرض بيانات serviceOrders في الكونسول
